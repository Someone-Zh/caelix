...
use crate::state::{App, TuiMessageType, NotificationType, AppView};
...
fn render_task_list(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let items: Vec<ListItem> = app.tasks.iter().map(|task| {
        ...
        // 从 payload 中提取任务类型名称
        let task_type_name = if let Ok(payload) = serde_json::from_str::<serde_json::Value>(&task.payload) {
...
fn render_notification_history_list(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let items: Vec<ListItem> = app.notifications_history.iter().enumerate().map(|(_idx, msg)| {
        ...
        let (icon, style) = match msg.r#type {
            RuntimeNotificationType::Info => ("ℹ️ ", Style::default().fg(Color::Rgb(86, 156, 214))),
            RuntimeNotificationType::Success => ("✅ ", Style::default().fg(Color::Rgb(78, 201, 176))),
            RuntimeNotificationType::Error => ("❌ ", Style::default().fg(Color::Rgb(212, 212, 212))),
            RuntimeNotificationType::Warning => ("⚠️ ", Style::default().fg(Color::Rgb(197, 134, 192))),
        };
