...
use crate::enhancement::commands::Command;
...
impl CommandManager {
    ...
    /// 根据类型过滤命令
    pub async fn get_by_type(&self, cmd_type: &crate::enhancement::commands::CommandType) -> Vec<Command> {
        let commands = self.commands.read().await;
        commands.iter()
            .filter(|c| c.r#type == *cmd_type)
            .cloned()
            .collect()
    }
}
