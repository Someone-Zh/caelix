use caelix_api::commands::{Command, CommandType};
...
pub async fn load_commands_from_directory(directory_path: &str) -> Result<Vec<Command>, String> {
        ...
        .filter_map(|e: Result<walkdir::DirEntry, walkdir::Error>| e.ok())
    {
        ...
        // 只处理 .md 文件
        if path.is_file() && path.extension().and_then(|s: &std::ffi::OsStr| s.to_str()) == Some("md") {
...
pub async fn register_all_commands(
    directory_path: &str,
    command_manager: &crate::managers::CommandManager,
